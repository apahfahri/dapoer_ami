...
class _DetailPesananPageState extends State<DetailPesananPage> {
  ...
  Future<Map<String, double>> _calculateTotalIngredients(Pesanan pesanan) async {
    Map<String, double> aggregatedIngredients = {};

    for (var itemPesanan in pesanan.items) {
      DocumentSnapshot menuDoc = await _firestore.collection('menu').doc(itemPesanan.menuId).get();
      
      if (menuDoc.exists) {
        final menuData = Menu.fromFirestore(menuDoc as DocumentSnapshot<Map<String, dynamic>>);
        
        for (var bahanResep in menuData.bahan) {
          double totalKuantitas = bahanResep.kuantitasPakai * itemPesanan.jumlah;
          String key = '${bahanResep.namaBahan} (${bahanResep.satuanPakai})';
          
          if (aggregatedIngredients.containsKey(key)) {
            aggregatedIngredients[key] = aggregatedIngredients[key]! + totalKuantitas;
          } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<DocumentSnapshot>(
        stream: _firestore.collection('pesanan').doc(widget.pesananId).snapshots(),
        builder: (context, snapshot) {
          ...
          final pesanan = Pesanan.fromFirestore(snapshot.data! as DocumentSnapshot<Map<String, dynamic>>);
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildSection("Informasi Pelanggan", _buildPelangganSection(pesanan)),
                _buildSection("Item yang Dipesan", _buildItemsSection(pesanan)),
                _buildSection("Ringkasan Pembayaran", _buildBiayaSection(pesanan, currency)),
              ],
            ),
          );
        },
        ...
        builder: (context, snapshot) {
          ...
          return Column(
            ...
            children: ingredients.entries.map((entry) {
              final formattedValue = entry.value.toStringAsFixed(1).replaceAll(RegExp(r'\.0$'), '');
              ...
              return Padding(
                ...
                child: Row(
                  ...
                  children: [
                    Text('- $namaBahan'),
                    Text('$formattedValue $satuan', style: const TextStyle(fontWeight: FontWeight.bold)),
                  ],
                ),
              );
            }).toList(),
  ...
  return ExpansionTile(
    ...
    childrenPadding: const EdgeInsets.fromLTRB(16, 0, 16, 16), // Padding diatur agar lebih rapi
